...
class PHPExcel_Shared_OLE_PPS_Root extends PHPExcel_Shared_OLE_PPS {
    ...
    public function __construct($time_1st, $time_2nd, $raChild) {
        ...
        parent::__construct(
                null, PHPExcel_Shared_OLE::Asc2Ucs('Root Entry'), PHPExcel_Shared_OLE::OLE_PPS_TYPE_ROOT, null, null, null, $time_1st, $time_2nd, null, $raChild);
    }
    ...
    public function _saveHeader($iSBDcnt, $iBBcnt, $iPPScnt) {
        ...
        // Save Header
        fwrite($FILE, "\xD0\xCF\x11\xE0\xA1\xB1\x1A\xE1"
                . "\x00\x00\x00\x00"
                . "\x00\x00\x00\x00"
                . "\x00\x00\x00\x00"
                . "\x00\x00\x00\x00"
        ...
        for ($i = 0; $i < $iCount; ++$i) {
            if ($raList[$i]->Type != PHPExcel_Shared_OLE::OLE_PPS_TYPE_DIR) {
                ...
                if (($raList[$i]->Size >= PHPExcel_Shared_OLE::OLE_DATA_SIZE_SMALL) ||
                    ...
                    $iStBlk += (floor($raList[$i]->Size / $this->_BIG_BLOCK_SIZE) +
                            (($raList[$i]->Size % $this->_BIG_BLOCK_SIZE) ? 1 : 0));
                }
