					...
					<div class="wrapper bg-silver-lighter">
						...
						<div class="btn-toolbar">
							...
							<div class="btn-group ml-auto">
								<label class="mb-0" style="font-size: 18px;"><i class="fa fa-envelope"></i> Sent Mails</label>
							</div>
							<!-- end btn-group -->
						</div>
						<!-- end btn-toolbar -->
												...
												<a href="javascript:;" class="email-user bg-grey mr-4">
													<script>
														$(document).ready(function(){
															...
															var PprofileImage = $('#ProfileSentMails<?=$mailsent->id?>').text(intials);
														});
													</script>
													<?php if(!empty($profile_image)){ ?>
														<img src="<?=base_url($profile_image)?>" class="fab text-white">
													<?php }else{ ?>
														<span id="ProfileSentMails<?=$mailsent->id?>" style="" class="ProfileSentMails text-uppercase"></span>
													<?php } ?>
												</a>
				...
				$(document).ready(function () {
					...
					$("#deleteinboxmails").click(function () {
						...
						if(checkinboxdata.length > 0){
							...
							$.ajax({
								url:"<?=base_url('user/mail/trash/permentdeletesentmails'); ?>",
								method:"POST",
								data:{delete_inboxdata:checkedInboxvalues,request_type:'deletesentmail'},
								dataType:'json',
