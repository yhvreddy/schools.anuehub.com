				...
				<?php if(count($mailsents) > 0){ ?>
					...
					<div class="wrapper bg-silver-lighter">
						...
						<div class="btn-toolbar">
							...
							<!-- begin btn-group -->
							<div class="btn-group">
								<button class="btn btn-sm btn-white" id="deleteinboxmails" data-email-action="delete"><i class="fa t-plus-1 fa-trash f-s-14 m-r-3"></i> <span class="hidden-xs">Delete forever</span></button>
								<button class="btn btn-sm btn-white" id="ImportantInboxMails" data-email-action="important"><i class="fa t-plus-1 fa-inbox f-s-14 m-r-3"></i> <span class="hidden-xs">Move to Inbox</span></button>
							</div>
							<!-- end btn-group -->
							<!-- begin btn-group -->
							<div class="btn-group ml-auto">
								<label class="mb-0" style="font-size: 18px;"><i class="fa fa-trash"></i> Trash Mails</label>
							</div>
							<!-- end btn-group -->
						</div>
						<!-- end btn-toolbar -->
												...
												<li class="list-group-item">
													...
													<div class="email-info">
														<a href="<?=base_url('user/mail/details/'.$mailsent->mail_ids.'/'.$mailsent->branch_id.'/'.$mailsent->school_id.'/sent-mail?id='.$mailsent->mail_id.'&inbox_id='.$mailsent->id)?>">
															...
															<span class="email-sender"><?=$name?></span>
															<span class="email-desc"><?=$mailsent->subject?></span>
										...
										$(document).ready(function () {
											...
											$("#deleteinboxmails").click(function () {
												...
												if(checkinboxdata.length > 0){
													...
													$.ajax({
														url:"<?=base_url('user/mail/trash/permentdeletemails'); ?>",
														method:"POST",
														data:{delete_inboxdata:checkedInboxvalues,request_type:'permentdelete'},
														dataType:'json',
											...
											$("#ImportantInboxMails").click(function () {
												...
												if(checkinboxdata.length > 0){
													...
													$.ajax({
														...
														method:"POST",
														data:{delete_inboxdata:checkedInboxvalues,request_type:'restoremails'},
														dataType:'json',
														success:function(inboxresponcedata)
														{
															console.log(inboxresponcedata);
